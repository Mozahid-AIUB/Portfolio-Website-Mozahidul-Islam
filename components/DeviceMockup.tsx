...
function Aura({ cover }: { cover: string }) {
  return (
    <div
      className="absolute -inset-5 rounded-[3rem] opacity-25 blur-3xl"
...
export function DeviceMockup({ project }: { project: Project }) {
  if (project.kind === "mobile") {
    return (
      <div className="relative mx-auto w-52 sm:w-60">
        <Aura cover={project.cover} />
        <div className="relative rounded-[2.4rem] border border-line bg-surface p-2 shadow-2xl shadow-black/40">
  ...
  const single =
    project.screenshots?.length === 1 && project.screenshotSize
  ...
  return (
    <div className="relative">
      <Aura cover={project.cover} />
      <div className="relative overflow-hidden rounded-xl border border-line bg-surface shadow-2xl shadow-black/40">
